...
class CheckUpListAutoCuidadoScreen extends StatefulWidget {
  String listautocuidadotitle,
      listautocuidadotitleABV,
      listautocuidadodescription;
  CheckUpListAutoCuidadoScreen(
      {super.key,
      required this.listautocuidadotitle,
      required this.listautocuidadotitleABV,
      required this.listautocuidadodescription});

  @override
  State<CheckUpListAutoCuidadoScreen> createState() => _CheckUpListAutoCuidadoScreenState();
}

class _CheckUpListAutoCuidadoScreenState extends State<CheckUpListAutoCuidadoScreen> {
  ...
  void initState() {
    super.initState();
    fromAsset('assets/documents/checkLists/IamInControl_Checklist_AutoCuidado_PT.pdf',
            'IamInControl_Checklist__AutoCuidado_PT.pdf')
        .then((f) {
      setState(() {
        pathPDF = f.path;
      });
                  ...
                  Row(
                    children: [
                      Padding(
                        ...
                        child: Text(widget.listautocuidadotitleABV,
                            style: TextStyle(
                                fontSize: 30,
                                color: Colors.black,
                                fontWeight: FontWeight.bold)),
                  ...
                  Row(
                    children: [
                      Padding(
                        ...
                        child: Text(widget.listautocuidadotitle,
                            style: TextStyle(
                              fontSize: 20,
                              color: Colors.black,
                            )),
                              ...
                              child: Column(
                                children: <Widget>[
                                  Padding(
                                    ...
                                    child: Text(
                                        widget.listautocuidadodescription,
                                        style: TextStyle(
                                            fontFamily: 'lato',
                                            fontSize: 17,
                                            color: Colors.black)),
                    ...
                    child: GestureDetector(
                      onTap: () {
                        ...
                        Navigator.push(context,
                          ...
                          return CheckUpListAutoCuidadoQuestions();
                        }));
                      },
                      child: Padding(
                        padding: const EdgeInsets.only(
...
class _PDFScreenState extends State<PDFScreen> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Documentação Check List Auto-Cuidado"),
        actions: <Widget>[
          IconButton(
...
class CheckUpListAutoCuidadoQuestions extends StatefulWidget {
  const CheckUpListAutoCuidadoQuestions({super.key});

  @override
  State<CheckUpListAutoCuidadoQuestions> createState() =>
      _CheckUpListAutoCuidadoQuestionsState();
}

class _CheckUpListAutoCuidadoQuestionsState
    extends State<CheckUpListAutoCuidadoQuestions> {
  late Future _questions;

  Future<List<Option>> getData() async {
    return getQuestionsListAutoCuidado();
  }
  ...
  String? listautocuidadoResultText = "";
          ...
          setState(() {
            if (snapshot.exists) {
              setState(() {
                ...
                listautocuidadoResultText = snapshot.data()!["resultText"];
              });
            } else {}
          });
      },
    ...
    if (index == questionsLength - 1) {
      ...
      showDialog(
          ...
          builder: ((context) => SmallerResultBox(
                listTitle: listautocuidadoTitleABV,
  ...
  Future setResult(int questionsLength) async {
    if (score >= 0) {
      FinalTextScore = listautocuidadoResultText!;
      TextScore = '$score / 14';
    }
  }
